...
-- see all apartments and thier residents (if any)
-- left join 
...
select a.apartment_id, a.apartment_number from residents r  
right join apartments a on r.apartment_id = a.apartment_id 
where  r.resident_id is null;


-- count the residents per apartment 

select a.apartment_number, count(r.resident_id) from residents r  
right join apartments a on r.apartment_id = a.apartment_id 
group by a.apartment_number;

-- list all apartments with residents and maintainenece
...
select a.apartment_id , count(m.request_id)
from apartments a 
left join maintenance_requests m on a.apartment_id = m.apartment_id group by a.apartment_id;
