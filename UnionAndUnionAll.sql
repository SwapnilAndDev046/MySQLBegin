...
-- union 
select *,'Employee' As contact_type from branch_employees 
union 
select *, 'Employee-premium' As contact_type from headquarters_employees;
...
-- get all employees sorted by there last name
...
-- find all departments that exists in both headquarters and branch offices

select *,count(*) from (select distinct department from branch_employees 
union all select distinct department from headquarters_employees) 
as combined group by department having count(*) = 2;
